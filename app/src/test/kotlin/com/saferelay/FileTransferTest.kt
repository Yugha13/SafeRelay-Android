...
class FileTransferTest {
    ...
    fun `encode and decode file packet with all fields should preserve data`() {
        ...
        val originalPacket = SafeRelayFilePacket(
            fileName = "test.png",
            mimeType = "image/png",
            fileSize = 1024000,
        ...
        val decoded = SafeRelayFilePacket.decode(encoded!!)
    ...
    fun `encode file packet with filename should include filename TLV`() {
        ...
        val packet = SafeRelayFilePacket(
    ...
    fun `replaceFilePathInContent should correctly format content markers for different file types`() {
        ...
        val imageMessage = SafeRelayMessage(
            id = "test1",
            sender = "alice",
            senderPeerID = "12345678",
            content = "/data/user/0/com.saferelay.android/files/images/photo.jpg",
            type = SafeRelayMessageType.Image,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )

        val audioMessage = SafeRelayMessage(
            id = "test2",
            sender = "bob",
            senderPeerID = "87654321",
            content = "/data/user/0/com.saferelay.android/files/audio/voice.amr",
            type = SafeRelayMessageType.Audio,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )

        val fileMessage = SafeRelayMessage(
            id = "test3",
            sender = "charlie",
            senderPeerID = "11223344",
            content = "/data/user/0/com.saferelay.android/files/documents/document.pdf",
            type = SafeRelayMessageType.File,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )
        ...
        result = result.replace(
            "/data/user/0/com.saferelay.android/files/images/photo.jpg",
            "[image] photo.jpg"
        )
