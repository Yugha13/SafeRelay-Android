package com.saferelay.android.protocol
...
enum class MessageType(val value: UByte) {
    ...
    SOS_RELAY(0x30u); // SOS store-and-forward relay
...
data class SafeRelayPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = SafeRelayPacket(
            version = version,
            type = type,
            senderID = senderID,
            ...
            route = route,
            ttl = com.saferelay.android.util.AppConstants.SYNC_TTL_HOPS // Use fixed TTL=0 for signing to ensure relay compatibility
        )
        return BinaryProtocol.encode(unsignedPacket)
    }

    companion object {
        fun fromBinaryData(data: ByteArray): SafeRelayPacket? {
...
object BinaryProtocol {
    ...
    fun encode(packet: SafeRelayPacket): ByteArray? {
        try {
    ...
    fun decode(data: ByteArray): SafeRelayPacket? {
     ...
     */
    private fun decodeCore(raw: ByteArray): SafeRelayPacket? {
        try {
