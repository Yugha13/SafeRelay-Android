...
class PrivateChatManager(
    ...
    private val unreadReceivedMessages = mutableMapOf<String, MutableList<SafeRelayMessage>>()
    ...
    fun startPrivateChat(peerID: String, meshService: BluetoothMeshService): Boolean {
        if (isPeerBlocked(peerID)) {
            ...
            val systemMessage = SafeRelayMessage(
                sender = "system",
    ...
    fun handleIncomingPrivateMessage(message: SafeRelayMessage) {
        handleIncomingPrivateMessage(message, suppressUnread = false)
    }

    fun handleIncomingPrivateMessage(message: SafeRelayMessage, suppressUnread: Boolean) {
    ...
    fun sendReadReceiptsForPeer(peerID: String, meshService: BluetoothMeshService) {
        ...
        val chats = try { state.getPrivateChatsValue() } catch (_: Exception) { emptyMap<String, List<SafeRelayMessage>>() }
    ...
    private fun consolidateNostrTempConversationIfNeeded(targetPeerID: String) {
        ...
        try {
            ...
            val npub = com.saferelay.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(noiseKeyBytes)
            if (npub != null) {
