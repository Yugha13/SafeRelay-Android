...
import com.saferelay.android.model.SafeRelayFilePacket
...
class MediaSendingManager(
    ...
    companion object {
        ...
        private const val MAX_FILE_SIZE = com.saferelay.android.util.AppConstants.Media.MAX_FILE_SIZE_BYTES // 50MB limit
    }
    ...
    fun sendVoiceNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            val filePacket = SafeRelayFilePacket(
                fileName = file.name,
            ...
            if (toPeerIDOrNull != null) {
                sendPrivateFile(toPeerIDOrNull, filePacket, filePath, SafeRelayMessageType.Audio)
            } else {
                sendPublicFile(channelOrNull, filePacket, filePath, SafeRelayMessageType.Audio)
            }
        } catch (e: Exception) {
    ...
    fun sendImageNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            if (toPeerIDOrNull != null) {
                sendPrivateFile(toPeerIDOrNull, filePacket, filePath, SafeRelayMessageType.Image)
            } else {
                sendPublicFile(channelOrNull, filePacket, filePath, SafeRelayMessageType.Image)
            }
        } catch (e: Exception) {
    ...
    fun sendFileNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            val mimeType = try { 
                com.saferelay.android.features.file.FileUtils.getMimeTypeFromExtension(file.name) 
            } catch (_: Exception) { 
            ...
            val messageType = when {
                mimeType.lowercase().startsWith("image/") -> SafeRelayMessageType.Image
                mimeType.lowercase().startsWith("audio/") -> SafeRelayMessageType.Audio
                else -> SafeRelayMessageType.File
            }
    ...
    private fun sendPrivateFile(
        toPeerID: String,
        filePacket: SafeRelayFilePacket,
        filePath: String,
        messageType: SafeRelayMessageType
    ) {
        ...
        messageManager.updateMessageDeliveryStatus(
            msg.id,
            com.saferelay.android.model.DeliveryStatus.PartiallyDelivered(0, 100)
        )
     ...
     */
    fun handleTransferProgressEvent(evt: com.saferelay.android.mesh.TransferProgressEvent) {
        val msgId = synchronized(transferMessageMap) { transferMessageMap[evt.transferId] }
        if (msgId != null) {
            if (evt.completed) {
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.saferelay.android.model.DeliveryStatus.Delivered(to = "mesh", at = java.util.Date())
                )
                synchronized(transferMessageMap) {
                ...
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.saferelay.android.model.DeliveryStatus.PartiallyDelivered(evt.sent, evt.total)
                )
            }
        }
    }
