...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    suspend fun handleNoiseEncrypted(routed: RoutedPacket) {
        ...
        try {
            ...
            val noisePayload = com.saferelay.android.model.NoisePayload.decode(decryptedData)
            ...
            when (noisePayload.type) {
                com.saferelay.android.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    // Decode TLV private message exactly like iOS
                    val privateMessage = com.saferelay.android.model.PrivateMessagePacket.decode(noisePayload.data)
                    if (privateMessage != null) {
                        ...
                        // Create SafeRelayMessage - preserve source packet timestamp
                ...
                com.saferelay.android.model.NoisePayloadType.FILE_TRANSFER -> {
                    // Handle encrypted file transfer; generate unique message ID
                    val file = com.saferelay.android.model.SafeRelayFilePacket.decode(noisePayload.data)
                    if (file != null) {
                        Log.d(TAG, "🔓 Decrypted encrypted file from $peerID: name='${file.fileName}', size=${file.fileSize}, mime='${file.mimeType}'")
                        val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                        val savedPath = com.saferelay.android.features.file.FileUtils.saveIncomingFile(appContext, file)
                        val message = SafeRelayMessage(
                            id = uniqueMsgId,
                            sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                            content = savedPath,
                ...
                com.saferelay.android.model.NoisePayloadType.DELIVERED -> {
                ...
                com.saferelay.android.model.NoisePayloadType.READ_RECEIPT -> {
                ...
                com.saferelay.android.model.NoisePayloadType.VERIFY_CHALLENGE -> {
                ...
                com.saferelay.android.model.NoisePayloadType.VERIFY_RESPONSE -> {
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = com.saferelay.android.model.NoisePayload(
                type = com.saferelay.android.model.NoisePayloadType.DELIVERED,
    ...
    suspend fun handleAnnounce(routed: RoutedPacket): Boolean {
        ...
        if (age > com.saferelay.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS) {
            Log.w(TAG, "Ignoring stale ANNOUNCE from ${peerID.take(8)} (age=${age}ms > ${com.saferelay.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS}ms)")
            return false
        }
        ...
        try {
            val neighborsOrNull = com.saferelay.android.services.meshgraph.GossipTLV.decodeNeighborsFromAnnouncementPayload(packet.payload)
        ...
        try {
            ...
            if (response != null) {
                ...
                val responsePacket = SafeRelayPacket(
                    ...
                    payload = response,
                    signature = null,
                    ttl = com.saferelay.android.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS
                )
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        try {
            ...
            val isFileTransfer = com.saferelay.android.protocol.MessageType.fromValue(packet.type) == com.saferelay.android.protocol.MessageType.FILE_TRANSFER
            val file = com.saferelay.android.model.SafeRelayFilePacket.decode(packet.payload)
            if (file != null) {
                if (isFileTransfer) {
    ...
    private fun handleFavoriteNotificationFromMesh(content: String, fromPeerID: String) {
        try {
            ...
            if (noiseKey != null) {
                com.saferelay.android.favorites.FavoritesPersistenceService.shared.updatePeerFavoritedUs(noiseKey, isFavorite)
                if (npub != null) {
                    // Index by noise key and current mesh peerID for fast Nostr routing
                    com.saferelay.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKey(noiseKey, npub)
                    com.saferelay.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(fromPeerID, npub)
                }
                ...
                val sys = com.saferelay.android.model.SafeRelayMessage(
                    sender = "system",
...
interface MessageHandlerDelegate {
    ...
    // Packet operations
    fun sendPacket(packet: SafeRelayPacket)
    fun relayPacket(routed: RoutedPacket)
    fun getBroadcastRecipient(): ByteArray
    
    // Cryptographic operations
    fun verifySignature(packet: SafeRelayPacket, peerID: String): Boolean
    ...
    // Callbacks
    fun onMessageReceived(message: SafeRelayMessage)
