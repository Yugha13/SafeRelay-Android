...
class SecurityManager(private val encryptionService: EncryptionService, private val myPeerID: String) {
    ...
    companion object {
        ...
        private const val MESSAGE_TIMEOUT = com.saferelay.android.util.AppConstants.Security.MESSAGE_TIMEOUT_MS // 5 minutes (same as iOS)
        private const val CLEANUP_INTERVAL = com.saferelay.android.util.AppConstants.Security.CLEANUP_INTERVAL_MS // 5 minutes
        private const val MAX_PROCESSED_MESSAGES = com.saferelay.android.util.AppConstants.Security.MAX_PROCESSED_MESSAGES
        private const val MAX_PROCESSED_KEY_EXCHANGES = com.saferelay.android.util.AppConstants.Security.MAX_PROCESSED_KEY_EXCHANGES
    }
     ...
     */
    fun validatePacket(packet: SafeRelayPacket, peerID: String): Boolean {
        ...
        if (processedMessages.contains(messageID)) {
            ...
            val isFreshAnnounce = messageType == MessageType.ANNOUNCE &&
                    packet.ttl >= com.saferelay.android.util.AppConstants.MESSAGE_TTL_HOPS
     ...
     */
    private fun generateMessageID(packet: SafeRelayPacket, peerID: String): String {
     ...
     */
    private fun verifyPacketSignature(packet: SafeRelayPacket, peerID: String): Boolean {
        try {
            ...
            if (MessageType.fromValue(packet.type) == MessageType.ANNOUNCE) {
                ...
                try {
                    val announcement = com.saferelay.android.model.IdentityAnnouncement.decode(packet.payload)
