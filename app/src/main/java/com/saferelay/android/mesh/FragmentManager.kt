...
import com.saferelay.android.model.FragmentPayload
...
class FragmentManager {
    ...
    companion object {
        ...
        private const val FRAGMENT_SIZE_THRESHOLD = com.saferelay.android.util.AppConstants.Fragmentation.FRAGMENT_SIZE_THRESHOLD // Matches iOS: if data.count > 512
        private const val MAX_FRAGMENT_SIZE = com.saferelay.android.util.AppConstants.Fragmentation.MAX_FRAGMENT_SIZE        // Matches iOS: maxFragmentSize = 469 
        private const val FRAGMENT_TIMEOUT = com.saferelay.android.util.AppConstants.Fragmentation.FRAGMENT_TIMEOUT_MS     // Matches iOS: 30 seconds cleanup
        private const val CLEANUP_INTERVAL = com.saferelay.android.util.AppConstants.Fragmentation.CLEANUP_INTERVAL_MS     // 10 seconds cleanup check
    }
     ...
     */
    fun createFragments(packet: SafeRelayPacket): List<SafeRelayPacket> {
        try {
        ...
        val fragments = mutableListOf<SafeRelayPacket>()
        ...
        for (index in fragmentChunks.indices) {
            ...
            val fragmentPacket = SafeRelayPacket(
     ...
     */
    fun handleFragment(packet: SafeRelayPacket): SafeRelayPacket? {
        ...
        try {
            ...
            if (fragmentMap != null && fragmentMap.size == fragmentPayload.total) {
                ...
                val originalPacket = SafeRelayPacket.fromBinaryData(reassembledData.toByteArray())
...
interface FragmentManagerDelegate {
    fun onPacketReassembled(packet: SafeRelayPacket)
}
