...
import com.saferelay.android.onboarding.BluetoothCheckScreen
...
import com.saferelay.android.onboarding.LocationCheckScreen
import com.saferelay.android.onboarding.LocationStatus
import com.saferelay.android.onboarding.LocationStatusManager
import com.saferelay.android.onboarding.OnboardingCoordinator
import com.saferelay.android.onboarding.OnboardingState
import com.saferelay.android.onboarding.PermissionExplanationScreen
import com.saferelay.android.onboarding.PermissionManager
import com.saferelay.android.ui.ChatScreen
import com.saferelay.android.ui.ChatViewModel
import com.saferelay.android.ui.OrientationAwareActivity
import com.saferelay.android.ui.theme.SafeRelayTheme
import com.saferelay.android.nostr.PoWPreferenceManager
...
class MainActivity : OrientationAwareActivity() {
    ...
    private val forceFinishReceiver = object : android.content.BroadcastReceiver() {
        override fun onReceive(context: android.content.Context, intent: android.content.Intent) {
            if (intent.action == com.saferelay.android.util.AppConstants.UI.ACTION_FORCE_FINISH) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val filter = android.content.IntentFilter(com.saferelay.android.util.AppConstants.UI.ACTION_FORCE_FINISH)
        if (android.os.Build.VERSION.SDK_INT >= 33) {
            registerReceiver(
                forceFinishReceiver,
                filter,
                com.saferelay.android.util.AppConstants.UI.PERMISSION_FORCE_FINISH,
                null,
        ...
        com.saferelay.android.service.AppShutdownCoordinator.cancelPendingShutdown()
        ...
        try { com.saferelay.android.service.MeshForegroundService.start(applicationContext) } catch (_: Exception) { }
        meshService = com.saferelay.android.service.MeshServiceHolder.getOrCreate(applicationContext)
    ...
    private fun initializeApp() {
        ...
        lifecycleScope.launch {
            try {
                ...
                com.saferelay.android.nostr.LocationNotesInitializer.initialize(this@MainActivity)
    ...
    private fun handleNotificationIntent(intent: Intent) {
        val shouldOpenPrivateChat = intent.getBooleanExtra(
            com.saferelay.android.ui.NotificationManager.EXTRA_OPEN_PRIVATE_CHAT, 
            false
        )
        
        val shouldOpenGeohashChat = intent.getBooleanExtra(
            com.saferelay.android.ui.NotificationManager.EXTRA_OPEN_GEOHASH_CHAT,
            false
        )
        
        when {
            shouldOpenPrivateChat -> {
                val peerID = intent.getStringExtra(com.saferelay.android.ui.NotificationManager.EXTRA_PEER_ID)
                val senderNickname = intent.getStringExtra(com.saferelay.android.ui.NotificationManager.EXTRA_SENDER_NICKNAME)
            ...
            shouldOpenGeohashChat -> {
                val geohash = intent.getStringExtra(com.saferelay.android.ui.NotificationManager.EXTRA_GEOHASH)
                ...
                if (geohash != null) {
                    ...
                    val level = when (geohash.length) {
                        7 -> com.saferelay.android.geohash.GeohashChannelLevel.BLOCK
                        6 -> com.saferelay.android.geohash.GeohashChannelLevel.NEIGHBORHOOD
                        5 -> com.saferelay.android.geohash.GeohashChannelLevel.CITY
                        4 -> com.saferelay.android.geohash.GeohashChannelLevel.PROVINCE
                        2 -> com.saferelay.android.geohash.GeohashChannelLevel.REGION
                        else -> com.saferelay.android.geohash.GeohashChannelLevel.CITY // Default fallback
                    }
                    val geohashChannel = com.saferelay.android.geohash.GeohashChannel(level, geohash)
                    val channelId = com.saferelay.android.geohash.ChannelID.Location(geohashChannel)
    ...
    private fun handleVerificationIntent(intent: Intent) {
        ...
        if (uri.scheme != "saferelay" || uri.host != "verify") return
