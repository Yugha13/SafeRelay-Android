...
import com.saferelay.android.model.PrivateMessagePacket
import com.saferelay.android.protocol.SafeRelayPacket
import com.saferelay.android.services.SeenMessageStore
import com.saferelay.android.ui.ChatState
import com.saferelay.android.ui.MeshDelegateHandler
import com.saferelay.android.ui.PrivateChatManager
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.saferelay.android.ui.DataManager
) {
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                if (!content.startsWith("saferelay1:")) return@launch

                val base64Content = content.removePrefix("saferelay1:")
                val packetData = base64URLDecode(base64Content) ?: return@launch
                val packet = SafeRelayPacket.fromBinaryData(packetData) ?: return@launch

                if (packet.type != com.saferelay.android.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                com.saferelay.android.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
    ...
    private suspend fun processNoisePayload(
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                val file = SafeRelayFilePacket.decode(payload.data)
                if (file != null) {
                    val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                    val savedPath = com.saferelay.android.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = SafeRelayMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
