package com.saferelay.android.nostr

import android.content.Context
import android.util.Log
import com.saferelay.android.model.ReadReceipt
...
class NostrTransport(
    ...
    companion object {
        ...
        private const val READ_ACK_INTERVAL = com.saferelay.android.util.AppConstants.Nostr.READ_ACK_INTERVAL_MS // ~3 per second (0.35s interval like iOS)
    ...
    fun sendPrivateMessage(
        ...
        transportScope.launch {
            try {
                ...
                // Strict: lookup the recipient's current SafeRelay peer ID using favorites mapping
                val recipientPeerIDForEmbed = try {
                    com.saferelay.android.favorites.FavoritesPersistenceService.shared
                ...
                val embedded = NostrEmbeddedSafeRelay.encodePMForNostr(
                    content = content,
                    messageID = messageID,
    ...
    private fun sendNextReadAck() {
        ...
        transportScope.launch {
            try {
                ...
                val ack = NostrEmbeddedSafeRelay.encodeAckForNostr(
    ...
    fun sendDeliveryAckGeohash(
        ...
        transportScope.launch {
            try {
                ...
                val embedded = NostrEmbeddedSafeRelay.encodeAckForNostrNoRecipient(
                    type = NoisePayloadType.DELIVERED,
                    messageID = messageID,
                    senderPeerID = senderPeerID
                )
    ...
    fun sendPrivateMessageGeohash(
        ...
        val geohash = sourceGeohash ?: run {
            val selected = try {
                com.saferelay.android.geohash.LocationChannelManager.getInstance(context).selectedChannel.value
            } catch (_: Exception) { null }
            if (selected !is com.saferelay.android.geohash.ChannelID.Location) {
        ...
        transportScope.launch {
            try {
                ...
                // Build embedded SafeRelay packet without recipient peer ID
                val embedded = NostrEmbeddedSafeRelay.encodePMForNostrNoRecipient(
                    content = content,
                    messageID = messageID,
                    senderPeerID = senderPeerID
                ) ?: run {
    ...
    private fun resolveNostrPublicKey(peerID: String): String? {
        try {
            ...
            com.saferelay.android.favorites.FavoritesPersistenceService.shared.findNostrPubkeyForPeerID(peerID)?.let { return it }

            // 2) Legacy path: resolve by noise public key association
            val noiseKey = hexStringToByteArray(peerID)
            val favoriteStatus = com.saferelay.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
            ...
            if (peerID.length == 16) {
                val fallbackStatus = com.saferelay.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)
